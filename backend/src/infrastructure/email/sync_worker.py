...
tocar a linha `user_integrations` (REQ-003) — não há nenhuma chamada a
`UserIntegrationRepositoryPort.save` neste módulo, então a garantia é
estrutural, não apenas comportamental.

Defesa contra `user_integrations` malformada: uma `email_accounts` pode
estar apontando para uma `user_integrations` com `integration_type !=
"imap"` (ex.: legados `smtp` stub com `config={}` criados via
`POST /api/integrations` antes deste MVP) ou com `config` que não bate
com `ImapIntegrationConfig` (ex.: truncado por migration parcial).
Nesses casos, a conta é marcada `status="error"` e o loop segue — uma
conta ruim não pode derrubar o worker inteiro (bug em produção
2026-08-10: `pydantic.ValidationError` em `ImapIntegrationConfig(**{})`
estava fora do `try/except` e matava o `poll_once`).
...
FetchMessages = Callable[[ImapIntegrationConfig, str, int], Awaitable[list[ParsedMessage]]]
...
class EmailSyncWorker:
    ...
    async def _poll_account(self, account: EmailAccount) -> None:
        ...
        if integration.integration_type != "imap":
            ...
            await self._mark_error(
                account,
                f"integration_type={integration.integration_type!r} (esperado 'imap')",
            )
            return
    ...
    async def _sync_folder(
        self, account: EmailAccount, config: ImapIntegrationConfig, folder: str
    ) -> None:
