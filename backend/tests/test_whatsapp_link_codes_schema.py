"""Testes de `src/infrastructure/persistence/whatsapp_link_codes_schema.py`.

Cobre a task `whatsapp-evolution-channel-task-linking-1`: DDL idempotente da
tabela `whatsapp_link_codes` (whatsapp-channel REQ-001), mesmo padrão de
`test_telegram_link_codes_schema.py`.
"""
from __future__ import annotations

import pytest

from src.infrastructure.persistence import whatsapp_link_codes_schema as schema
...
def test_ensure_schema_creates_whatsapp_link_codes_table(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "CREATE TABLE IF NOT EXISTS whatsapp_link_codes" in executed_sql
...
def test_ensure_schema_called_twice_does_not_raise(monkeypatch: pytest.MonkeyPatch) -> None:
    """whatsapp-evolution-channel-task-linking-1-unit-1."""
    fake_conn = _FakeConnection()
    monkeypatch.setattr(schema.psycopg, "connect", lambda *a, **kw: fake_conn)
