"""Bootstrap de `ensure_crm_schema` no lifespan do webapp."""
...
def _patch_lifespan_deps(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(webapp, "ensure_email_schema", lambda conninfo: None)
...
def test_webapp_lifespan_calls_crm_schema_ensure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """WHEN lifespan startup runs THEN ensure_crm_schema is called with POSTGRES_URI."""
    calls: list[str] = []
    monkeypatch.setenv("POSTGRES_URI", "postgresql://crm-webapp")
    ...
    assert calls == ["postgresql://crm-webapp"]
