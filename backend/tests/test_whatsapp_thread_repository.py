"""Testes de `src/infrastructure/whatsapp/thread_repository.py`.

`get_or_create_thread_id`/`set_active_thread`/`create_thread_for_number` usam
o fake transacional `_TxCursor` (mesmo padrão de
`test_telegram_thread_repository.py`) — `thread_id` é a PK, `phone_number`
pode se repetir entre linhas desde `whatsapp-slash-commands` (ver migração em
`test_whatsapp_schema.py`).
"""
...
# create_thread_for_number — mesmo padrão de test_telegram_thread_repository.py.
# ============================================================================
...
class _TxCursor:
    ...
    def execute(self, query: str, params: tuple) -> None:
        q = query.strip()
        if q.startswith("SELECT thread_id FROM whatsapp_threads") and "active = TRUE" in q:
            (phone_number,) = params
            match = next(
                (r for r in self._rows if r["phone_number"] == phone_number and r["active"]),
                None,
            )
            self._last_result = (match["thread_id"],) if match is not None else None
        elif q.startswith("SELECT 1 FROM whatsapp_threads") and "thread_id = %s" in q:
            phone_number, thread_id = params
            exists = any(
                r["phone_number"] == phone_number and r["thread_id"] == thread_id
                for r in self._rows
            )
            self._last_result = (1,) if exists else None
        elif q.startswith("SELECT 1 FROM whatsapp_threads") and "title = %s" in q:
            phone_number, title = params
            exists = any(
                r["phone_number"] == phone_number and r["title"] == title for r in self._rows
            )
            self._last_result = (1,) if exists else None
        elif q.startswith("UPDATE whatsapp_threads SET active = FALSE") and "!=" in q:
            phone_number, exclude_thread_id = params
            for r in self._rows:
                if (
                    ...
                    r["active"] = False
        elif q.startswith("UPDATE whatsapp_threads SET active = FALSE"):
            (phone_number,) = params
            for r in self._rows:
                if r["phone_number"] == phone_number and r["active"]:
                    r["active"] = False
        elif q.startswith("UPDATE whatsapp_threads SET active = TRUE"):
            phone_number, thread_id = params
            target = next(
                (
                    r
                    for r in self._rows
                    if r["phone_number"] == phone_number and r["thread_id"] == thread_id
                ),
                None,
            )
            ...
            phone_number, thread_id, title = params
            self._rows.append(
                {
...
def _tx_row(
    phone_number: str, thread_id: str, title: str | None, active: bool
) -> dict[str, object]:
    return {
        "phone_number": phone_number,
        "thread_id": thread_id,
...
# ---------------------------------------------------------------------------
# get_or_create_thread_id — REQ-002 (whatsapp-channel delta, refatorado para
# honrar `active`)
# ---------------------------------------------------------------------------


def test_get_or_create_thread_id_creates_active_thread_for_unseen_number(
    ...
    second = thread_repository.get_or_create_thread_id("+5511999990000")
...
def test_get_or_create_thread_id_creates_new_when_no_row_is_active(
    ...
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    rows: list[dict[str, object]] = [_tx_row("123", "T-old", None, False)]
    monkeypatch.setattr(thread_repository.psycopg, "connect", _connect_tx_factory(rows))

    thread_id = thread_repository.get_or_create_thread_id("123")
...
# ---------------------------------------------------------------------------
# set_active_thread — REQ-007 (whatsapp-slash-commands-spec)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# create_thread_for_number — REQ-002 (whatsapp-slash-commands-spec)
# ---------------------------------------------------------------------------


def test_create_thread_for_number_deactivates_previous_and_inserts_active(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    ...
    thread_id = thread_repository.create_thread_for_number("123", title="pagamentos")
...
def test_create_thread_for_number_duplicate_title_raises_without_inserting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    ...
    with pytest.raises(ValueError):
        thread_repository.create_thread_for_number("123", title="pagamentos")

    assert len(rows) == 1


def test_create_thread_for_number_without_title_inserts_null_title(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    rows: list[dict[str, object]] = []
    ...
    thread_id = thread_repository.create_thread_for_number("123")
...
# ---------------------------------------------------------------------------
# "Restart preserva thread ativa" — REQ-007 (whatsapp-slash-commands-spec)
# ---------------------------------------------------------------------------
...
def test_get_or_create_thread_id_survives_new_connection_after_set_active(
    ...
    # Simula um restart do processo: uma conexão nova, mesmas linhas por baixo.
...
# ============================================================================
# Fake multi-row table for list_threads_for_number / update_thread_title /
# get_thread_by_title (whatsapp-slash-commands-task-repository-1) — mesmo
# padrão de test_telegram_thread_repository.py, dispatch por formato de SQL
# (LIMIT 20 / UPDATE / SELECT simples), já que estas operam sobre várias
# linhas por phone_number, diferente do mapeamento único usado acima.
# ============================================================================
...
class _MultiRowCursor:
    ...
    def execute(self, query: str, params: tuple) -> None:
        ...
        if query.startswith("SELECT") and "LIMIT 20" in query:
            (phone_number,) = params
            matches = [r for r in self._rows if r["phone_number"] == phone_number]
            ...
            phone_number, title, exclude_thread_id = params
            conflict = any(
                r["phone_number"] == phone_number
                and r["title"] == title
                and r["thread_id"] != exclude_thread_id
            ...
            title, phone_number, thread_id = params
            target = next(
                (
                    r
                    for r in self._rows
            ...
            match = next(
                (
                    r
                    for r in self._rows
                    if r["phone_number"] == phone_number and r["title"] == title
                ),
                None,
            )
            self._last_result = (
...
def _row(
    phone_number: str, thread_id: str, title: str | None, active: bool, created_at: datetime
) -> dict[str, object]:
    return {
        "phone_number": phone_number,
        "thread_id": thread_id,
...
# ---------------------------------------------------------------------------
# list_threads_for_number — REQ-005 (whatsapp-slash-commands-spec)
# ---------------------------------------------------------------------------


def test_list_threads_for_number_orders_by_created_at_desc(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    ...
    result = thread_repository.list_threads_for_number("123")

    assert [r["thread_id"] for r in result] == ["A", "B", "C"]
    assert set(result[0].keys()) == {"thread_id", "title", "active", "created_at"}


def test_list_threads_for_number_limits_to_20(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    now = datetime.now(UTC)
    rows = [
...
def test_list_threads_for_number_empty_returns_empty_list(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    monkeypatch.setattr(thread_repository.psycopg, "connect", _connect_multi_row_factory([]))

    result = thread_repository.list_threads_for_number("999")

    assert result == []


# ---------------------------------------------------------------------------
# update_thread_title — REQ-003 (whatsapp-slash-commands-spec)
# ---------------------------------------------------------------------------
...
def test_update_thread_title_duplicate_raises_value_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
    rows = [
...
# ---------------------------------------------------------------------------
# get_thread_by_title — REQ-004 (whatsapp-slash-commands-spec)
# ---------------------------------------------------------------------------
...
def test_get_thread_by_title_same_title_different_number_returns_none(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("POSTGRES_URI", "postgresql://fake")
