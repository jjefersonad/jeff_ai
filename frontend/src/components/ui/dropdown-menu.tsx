...
/**
 * shadcn-style `DropdownMenu` primitive backed by `@radix-ui/react-dropdown-menu`.
 *
 * The other UI primitives in `frontend/src/components/ui/` follow the same
 * shape (Radix wrapper + `data-slot` attributes + `cn` from `@/lib/utils`).
 * This file adds the missing primitive needed by the `UserMenu` component
 * introduced in `frontend-menu-redesign` / task `components-3`.
 */
...
function DropdownMenuSubTrigger({
  ...
  return (
    <DropdownMenuPrimitive.SubTrigger
      ...
      className={cn(
        "flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[state=open]:bg-accent data-[state=open]:text-accent-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        inset && "pl-8",
        className
      )}
      {...props}
