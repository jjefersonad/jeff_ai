  ...
  async rewrites() {
    return [
      ...
      // `frontend`, que resolve nomes da rede Docker — não `localhost`.
      {
        ...
        destination: "http://backend:8003/api/images/:path*",
      },
      {
        source: "/api/files/:path*",
        destination: "http://backend:8003/api/files/:path*",
      },
      {
        source: "/api/references",
        destination: "http://backend:8003/api/references",
      },
      {
        source: "/api/references/:path*",
        destination: "http://backend:8003/api/references/:path*",
      },
      // `/api/mcp/*` continua no container `image-server` (processo
      // isolado do grafo do agente — REQ-001 do `mcp-client`).
      {
        source: "/api/mcp/:path*",
        destination: "http://image-server:8083/api/mcp/:path*",
      },
    ];
  },
};
